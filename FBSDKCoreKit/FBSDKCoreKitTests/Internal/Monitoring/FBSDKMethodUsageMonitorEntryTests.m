...
- (void)testCreatingEntryWithMethodName
{
  ...
  XCTAssertNil([actual objectForKey:@"parameters"],
               @"Should not store parameters when none are provided");
}

- (void)testCreatingWithMethodNameAndParameters
{
  entry = [FBSDKMethodUsageMonitorEntry entryWithMethod:@selector(viewDidAppear:)
                                             parameters:@{@"animated":@YES}];

  NSDictionary *actual = [entry dictionaryRepresentation];

  XCTAssertEqualObjects([actual objectForKey:@"parameters"], @{@"animated": @YES},
                        @"Should track the parameters the method was called with");
}
...
- (void)testEncodingEntryWithMethodNameAndParameters
{
  TestCoder *coder = [TestCoder new];
  ...
  XCTAssertEqualObjects(coder.encodedObject[@"appID"], @"abc123",
                        @"Should include the superclass' encoding");
  XCTAssertEqualObjects(coder.encodedObject[@"parameters"], @{@"animated":@YES},
                        @"Should encode the parameters of the method call if they're available");
}
